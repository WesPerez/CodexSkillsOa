...
6. 后端改动 -> 调用 `oa-dev-verification-gate` 编译、IDEA DevTools reload、Logback/API 探活。
...
- 调用 `oa-real-browser-driver` 或 `oa-real-action-evidence` 时，传入需要真实页面证明的控件矩阵、样例 id、预期业务结果和安全停止条件。消费其 action evidence 状态，不把可见性当一致性。
- 调用 `oa-dev-verification-gate` 时，传入后端/Mapper/模板/配置改动和需要刷新的接口路径。消费其 runtime fresh/stale 证据后再决定是否继续真实页面验收。
...
4. 追踪老 OA 证据链。
5. 追踪新 OA 证据链。
6. 用真实数据库只读校验每条 SQL 涉及的对象、字段、函数、视图、同义词、当前 schema 和 SQL 可解析性。
7. 建立逐项对比矩阵。
8. 判定一致、偏差或无法判定。
9. 对确认偏差提出修复建议；若用户要求修复且风险可控，可直接修复。
10. 运行必要验证，报告已验证与未验证项。
...
- 如果需要修改代码，必须先检查 `git status --short`，只改本功能相关文件，不覆盖用户已有改动。
