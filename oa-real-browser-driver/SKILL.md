...
- 与 `oa-real-sql-gate`：请求真实 SQL 解析、样例发现、DML 测试数据构造、备份/读回/恢复和恢复验证；不得自行执行不受控写入。
...
- 超时、阻断、异常恢复、插件不可用、runtime stale、needs-data、manual authorization required 等状态出现。
...
3. 空白列表、无数据、筛选后 0 行、按钮不可见或没有可下载内容，都不能作为真实验收完成。必须继续执行以下之一：
   ...
   - 若生产/未知库、无授权、外部系统缺失或恢复计划不足，标为 `needs-data` / `manual authorization required` / `blocked-needs-safe-sample-or-rollback-plan`。
4. 构造或调整测试数据只能用于开发/测试库，且必须由 `oa-real-sql-gate` 记录库身份、影响表、原始快照、插入/更新/删除、预期影响行数、读回、浏览器验收后的恢复和恢复验证。除非用户明确要求保留测试数据，否则测试完必须恢复。
