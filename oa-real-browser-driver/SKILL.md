...
4. 接管或打开真实 Edge 标签页。
5. 对只读动作直接点击取证；对写入/下载副作用动作先交给 `oa-real-sql-gate`。
...
- 与 `oa-real-action-evidence`：交付健康 tab、样例 id、动作分类和预期结果；消费其 Network/console/可见消息/下载/DB 证据。
- 与 `oa-real-sql-gate`：请求真实 SQL 解析、样例发现、DML 备份/读回/恢复和恢复验证；不得自行执行不受控写入。
- 与 `oa-dev-verification-gate`：后端/Mapper/模板/配置变化后，请其证明编译、DevTools reload、Logback 和探活，再回到真实动作。

返回状态保持精确：`real-verified`、`real-verified read-only`、`real-verified write-readback-restored`、`partial-verified`、`source-verified`、`needs-data`、`manual authorization required`、`blocked-needs-safe-sample-or-rollback-plan`、`runtime-stale`、`tab-unhealthy-needs-browser-driver`、`not-applicable-no-sql`、`not-applicable-no-browser-surface`。

## 标签页管理

1. 先读取官方浏览器插件技能，确认可用连接方式。
2. 通过 `browser.user.openTabs()` 或等价官方扩展能力列出现有 Edge 标签页。
3. 按 URL、标题、路由、最近活动和用户描述选择最接近的 OA 标签页。
4. owner class 必须记录为：
   - `user-existing`
...
7. 结束时释放控制。只关闭 `current-session-created` 且不再需要、无交付依赖的标签页。其他标签页保留并在最终回复说明。
...
2. 用户明确授权浏览器记住的凭据时，只通过可见登录表单聚焦字段并选择可见 autofill；不要检查 cookies、local storage、密码库、profile 或 token 文件。
...
4. 没有合适账号且确认数据库为开发/测试库时，只有在 `oa-real-sql-gate` 提供精确快照、最小可逆变更、读回、立即恢复和恢复验证后，才允许临时权限数据调整。
5. 临时权限变更必须与产品代码变更分开记录，且禁止用于生产库或身份不明库。
6. 权限门依赖外部系统、深链数据库或未决业务负责人时，标为 `manual authorization required`，写清缺少的账号/字段/样例。
...
- 写入/下载副作用控件必须先通过 SQL gate。
- 范围内控件未点击时，必须标为 `source-verified`、`partial-verified`、`blocked-needs-safe-sample-or-rollback-plan`、`manual authorization required` 或 `not in scope`，不能泛称已验证。
...
2. 请求 URL、method、params/data、headers、responseType、content type 和响应状态。
3. 保存文件真实存在、大小大于 0、签名与类型匹配。
4. 文件内容包含代表性业务字段，例如项目号、标题、二维码/条码、Excel/Word 单元格。
5. 有写入副作用时，附 SQL gate 的读回和恢复证据，除非用户明确要求保留状态。
...
不要用“保存了文件”“编译过”直接当成后端已新鲜。必须有运行时证据。

## 报告要求

真实验收报告或最终摘要应包含：

- 环境 URL、前端路由、后端 base URL。
- 浏览器表面、tab owner class、是否新建/接管/保留/关闭。
- 登录账号/角色标签，不含密码。
- companyCode/schema、样例 id、筛选条件和时间戳。
- 每个动作的 URL、method、params/data、响应、可见消息、console、下载文件、DB 读回/恢复。
